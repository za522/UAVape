...
\toprule
\textbf{Dataset component} & \textbf{Scale / composition} & \textbf{Discussion relevance} \\
\midrule
...
\bottomrule
\end{tabularx}
\caption{Dataset-scale context for interpreting UAVape against broader litter-detection benchmarks.}
\label{tab:discussion_dataset_context}
\end{table}

\begin{table}[H]
\centering
\small
\begin{tabularx}{\textwidth}{p{0.16\textwidth} p{0.12\textwidth} p{0.16\textwidth} p{0.18\textwidth} X}
\toprule
\textbf{Study / dataset} & \textbf{UAV?} & \textbf{Reported metric} & \textbf{Task / target} & \textbf{Comparison nuance} \\
\midrule
...
\bottomrule
\end{tabularx}
\caption{Contextual comparison between UAVape V4 and reviewed litter-detection benchmarks.}
